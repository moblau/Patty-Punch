      ...
      apvts (*this, nullptr, "PattyPunchState", Params::createLayout())
{
    ...
    for(size_t i=0;i<lfoCount;++i)
        raw.lfos[i]={get(rates[i]),get(shapes[i]),get(pitches[i]),get(decays[i]),get(sources[i]),get(warps[i])};
...
DrumParameters PattyPunchAudioProcessor::snapshot() const
{
    ...
    p.kickLevel=juce::Decibels::decibelsToGain(raw.kickLevel->load());
    p.snareTune=raw.snareTune->load();p.snareDecay=raw.snareDecay->load()*.001f;p.snareSnappy=raw.snareSnappy->load()*.01f;
    p.snareTone=raw.snareTone->load();p.snareLow=raw.snareLow->load();p.snareCrack=raw.snareCrack->load();p.snareAir=raw.snareAir->load();
    p.snareLevel=juce::Decibels::decibelsToGain(raw.snareLevel->load());p.hatTune=raw.hatTune->load();
    p.hatDecay=raw.hatDecay->load()*.001f;p.hatTone=raw.hatTone->load()*.01f;p.hatHP=raw.hatHighPass->load();
    p.hatChoke=raw.hatChoke->load()>.5f;p.hatLevel=juce::Decibels::decibelsToGain(raw.hatLevel->load());
    for(size_t i=0;i<lfoCount;++i)
    {
        ...
        destination.pitchDepth=source.pitch->load();destination.decayDepth=source.decay->load()*.01f;
...
{
    static constexpr const char* labels[]{"C0-B1 | MIDI 24-47","C2-B2 | MIDI 48-59","C3-B3 | MIDI 60-71"};
...
{
    if(auto xml=getXmlFromBinary(d,n)){auto tree=juce::ValueTree::fromXml(*xml);if(tree.isValid()){Params::ensureWarblerState(tree);apvts.replaceState(tree);Params::ensureMidiProperties(apvts.state);}}
}
